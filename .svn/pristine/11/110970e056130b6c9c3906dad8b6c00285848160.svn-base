...
public class UserDaoSSHImpl extends HibernateDaoSupport implements UserDao {
	...
	public void addUser(User user) {
		...
		try {
			...
			throw new ApplicationException("该账号名已经存在！");
		}
		catch (Exception e) {
			System.out.println();
			throw new ApplicationException("该账号名已经存在！");
	...
	private DetachedCriteria genCriteriaByHelper(UserHelper helper){
		...
		criteria.addOrder(Order.desc("userNo"));
		return criteria;
	}
	...
	public User searchUserByNo(String userNo) {
		// TODO Auto-generated method stub
		return (User) this.getHibernateTemplate().find("from User where userNo="+userNo).get(0);
