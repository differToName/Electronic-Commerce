...
create table tb_goods_class(
	...
	CONSTRAINT TG_FK_TGP REFERENCES tb_goods_class(goods_class_no) ON DELETE CASCADE,
...
CREATE TABLE tb_goods(
  ...
  CONSTRAINT tg_fk_tgc REFERENCES tb_goods_class(goods_class_no) ON DELETE CASCADE,
  goods_inventory INT DEFAULT 0 NOT NULL ,  --商品库存量
  goods_price NUMBER(9,2),       --商品单价
  goods_picture BLOB,     --商品图片
  goods_description VARCHAR2(200),    --商品描述
...
create table tb_order(
	...
	audit_statu char(1) not null,--审核状态    N-未审核(默认) P-审核已通过  X-审核未通过
